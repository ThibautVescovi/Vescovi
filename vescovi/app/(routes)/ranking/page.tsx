    ...
    return (
        <div className="relative min-h-[calc(100vh-80px)] overflow-hidden bg-[radial-gradient(circle_at_top_left,rgba(250,204,21,0.18),transparent_28%),radial-gradient(circle_at_bottom_right,rgba(16,185,129,0.18),transparent_28%)]">
            ...
            <div className="relative z-10 mx-auto w-full max-w-4xl px-4 py-10 sm:px-8 sm:py-14">
                    ...
                    <div className="mb-10 grid gap-4 sm:grid-cols-3">
                        {rankings.slice(0, 3).map((entry) => {
                            ...
                            return (
                                ...
                                >
                                    {isMe && (
                                        <span className="absolute right-3 top-3 rounded-full bg-yellow-300 px-2 py-0.5 text-[10px] font-black text-emerald-950">
                                            Moi
                                        </span>
                                    )}
                ...
                {/* Full table */}
                {rankings.length === 0 ? (
                    <div className="rounded-[1.5rem] border border-white/10 bg-white/10 p-10 text-center text-emerald-50/70 shadow-xl backdrop-blur">
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className="px-6 py-4 text-lg">
                                                {isTop3 ? medals[entry.rank - 1] : <span className="font-bold text-emerald-50/60">{entry.rank}</span>}
                                            </td>
                                            <td className="px-6 py-4">
                                                ...
                                                {isMe && (
                                                    <span className="ml-2 rounded-full bg-yellow-300 px-2 py-0.5 text-[10px] font-black text-emerald-950">
                                                        Moi
                                                    </span>
                                                )}
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <span className={`text-xl font-black ${isTop3 ? "text-yellow-300" : "text-white"}`}>
                                                    {entry.totalPoints}
                                                </span>
                        ...
                        <ul className="flex flex-col divide-y divide-white/10 sm:hidden">
                            {rankings.map((entry) => {
                                ...
                                return (
                                    ...
                                    >
                                        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-emerald-950/50 text-xl font-black ring-1 ring-white/10">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <p className="font-bold text-white truncate">
                                                {entry.pronostiqueurName}
                                                {isMe && (
                                                    <span className="ml-1.5 rounded-full bg-yellow-300 px-1.5 py-0.5 text-[9px] font-black text-emerald-950">
                                                        Moi
                                                    </span>
                                                )}
                                            </p>
                ...
                {currentUserRank && currentUserRank.rank > 3 && (
                    <div className="mt-6 flex items-center gap-4 rounded-[1.25rem] border border-yellow-300/30 bg-yellow-300/10 px-5 py-4 shadow-lg shadow-black/10 backdrop-blur">
                        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-yellow-300/20 font-black text-yellow-300 ring-1 ring-yellow-300/30 text-lg">
